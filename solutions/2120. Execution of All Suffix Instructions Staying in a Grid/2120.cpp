 ...
 public:
  vector<int> executeInstructions(int n, vector<int>& startPos, string s) {
    ...
    for (int i = m - 1; i >= 0; --i) {
      ...
      reachY[y] = i;
      int out = INT_MAX;
      if (const auto it = reachX.find(x - uMost); it != cend(reachX))
        out = min(out, it->second);
      if (const auto it = reachX.find(x + dMost); it != cend(reachX))
        out = min(out, it->second);
      if (const auto it = reachY.find(y - lMost); it != cend(reachY))
        out = min(out, it->second);
      if (const auto it = reachY.find(y + rMost); it != cend(reachY))
        out = min(out, it->second);
