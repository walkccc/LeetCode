class Solution {
  public int maxFrequencyScore(int[] nums, int k) {
    ...
    for (int i = k; i < nums.length; ++i) {
      ...
      count.merge(leftNum, -1, Integer::sum);
      ...
      count.merge(rightNum, 1, Integer::sum);
      sum = (sum + modPow(rightNum, count.get(rightNum))) % kMod;
  ...
  private int modPow(long x, long n) {
    if (n == 0)
      return 1;
    if (n % 2 == 1)
      return (int) (x * modPow(x % kMod, (n - 1) % kMod) % kMod);
    return (int) (modPow(x * x % kMod, (n / 2) % kMod) % kMod);
  }
}
