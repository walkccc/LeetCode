...
class SegmentTree : RangeQuery<int> {
 public:
  ...
  // the index of the element in the array that needs to be updated.
  void update(int i, int index) override {
    update(0, 0, n, i, index);
  }

  // Returns the index k s.t. `nums[k]` is the smallest num in nums[i..j].
 ...
 private:
  ...
  vector<int> tree;        // tree[i] := max index stored in this node
