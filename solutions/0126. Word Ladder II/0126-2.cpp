    ...
    while (!forwardWords.empty() && !backwardWords.empty()) {
      ...
      for (const string& parent : forwardWords) {
        for (const string& child :
          ...
          // Should check if the `child` is in `backwardWords` since we erase
          // them in the beginning of the each while loop.
          ...
          // We've reached the end word since there's a word in `forwardWords`
          // connecting to a word in `backwardWords`. Note that we can't return
          // here since we need to completely explore this level.
