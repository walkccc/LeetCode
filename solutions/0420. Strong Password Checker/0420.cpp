class Solution {
 public:
  int strongPasswordChecker(string s) {
    ...
    int replaces = 0;  // # of replacements to deal with three repeating characters
    int oneSeq = 0;  // # of seqs that can be substituted with 1 deletions, (3k)-seqs
    int twoSeq = 0;  // # of seqs that can be substituted with 2 deletions, (3k + 1)-seqs
    ...
    if (n <= 20) return max(replaces, missing);
 ...
 private:
  int getMissing(const string& s) {
    int missing = 3;
    if (any_of(begin(s), end(s), [](char c) { return isupper(c); })) --missing;
    if (any_of(begin(s), end(s), [](char c) { return islower(c); })) --missing;
    if (any_of(begin(s), end(s), [](char c) { return isdigit(c); })) --missing;
    return missing;
  }
};