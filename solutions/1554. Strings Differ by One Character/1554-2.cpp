class Solution {
 public:
  bool differByOne(vector<string>& dict) {
    ...
    // Compute the hash without each character.
    // E.g. hash = "abc" = 26^2 * 'a' + 26 * 'b' + 'c'
    //   newHash = "a*c" = hash - 26 * 'b'
    int coefficient = 1;
    for (int j = m - 1; j >= 0; --j) {
 ...
 private:
  ...
  // Returns hash of `s`. Assume the length of `s` is m.
  // getHash(s) = 26^(m - 1) * s[0] + 26^(m - 2) * s[1] + ... + s[m - 1].
