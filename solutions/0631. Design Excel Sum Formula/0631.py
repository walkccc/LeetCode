...
class Excel:
  def __init__(self, height: int, width: str):
    self.sheet = [[Cell(0, None) for i in range(height)] for _ in range(ord(width) - ord('A') + 1)]
  ...
  def sum(self, row: int, column: str, numbers: List[str]) -> int:
    self.sheet[row - 1][ord(column) - ord('A')].posCount = self.__parse(numbers)
    return self.get(row, column)

  def __parse(self, numbers: List[str]):
    count = Counter()
    for n in numbers:
