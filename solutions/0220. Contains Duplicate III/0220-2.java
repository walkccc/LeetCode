  ...
  public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) {
    ...
    for (int i = 0; i < nums.length; ++i) {
      ...
      if (bucket.containsKey(key)) // current bucket
        return true;
      if (bucket.containsKey(key - 1) && Math.abs(bucket.get(key - 1) - num) < diff) // left adjacent bucket
        return true;
      if (bucket.containsKey(key + 1) && Math.abs(bucket.get(key + 1) - num) < diff) // right adjacent bucket
        return true;
      bucket.put(key, num);
      if (i >= k)
