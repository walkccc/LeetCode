class Solution {
  ...
  private void dfs(List<Character> candidates, StringBuilder mid, boolean[] used, StringBuilder path,
      List<String> ans) {
    if (path.length() == candidates.size()) {
      ans.add(path.toString() + mid + path.reverse().toString());
      path.reverse();
      return;
    }
