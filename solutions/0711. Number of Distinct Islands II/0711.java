class Solution {
  ...
  private void dfs(
      int[][] grid, int i, int j, boolean[][] seen, List<Pair<Integer, Integer>> island) {
  ...
  private List<Pair<Integer, Integer>> normalize(List<Pair<Integer, Integer>> island) {
    ...
    for (Pair<Integer, Integer>[] point : points)
      Arrays.sort(point,
          (a, b)
              -> a.getKey() == b.getKey() ? a.getValue() - b.getValue() : a.getKey() - b.getKey());

    // normalize each point by minus point[1:] w/ point[0]
    for (Pair<Integer, Integer>[] point : points) {
      for (int i = 1; i < island.size(); ++i)
        point[i] = new Pair<>(
            point[i].getKey() - point[0].getKey(), point[i].getValue() - point[0].getValue());
      point[0] = new Pair<>(0, 0);
    }
