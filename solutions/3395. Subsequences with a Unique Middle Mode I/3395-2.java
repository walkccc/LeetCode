  ...
  public int subsequencesWithMiddleMode(int[] nums) {
    ...
    for (int i = 0; i < nums.length; ++i) {
      ...
      for (final int b : getUniqueNums(p, s)) {
        ...
        // [a b] a [b c]
        subtract = (int) ((subtract + (long) pa * pb * sb * (r - sa - sb)) % kMod);

        // [b c] a [a b]
        subtract = (int) ((subtract + (long) sa * sb * pb * (l - pa - pb)) % kMod);

        // [b b] a [a c]
        subtract = (int) ((subtract + (long) nC2(pb) * sa * (r - sa - sb)) % kMod);

        // [a c] a [b b]
        subtract = (int) ((subtract + (long) nC2(sb) * pa * (l - pa - pb)) % kMod);

        // [b b] a [a b]
        subtract = (int) ((subtract + (long) nC2(pb) * sa * sb) % kMod);

        // [a b] a [b b]
        subtract = (int) ((subtract + (long) nC2(sb) * pa * pb) % kMod);
