...
class Solution {
  public int minimumMoves(int[][] grid) {
    ...
    Queue<int[]> q = new ArrayDeque<>(Arrays.asList(new int[] {0, 0, Pos.HORIZONTAL.ordinal()}));
    ...
    while (!q.isEmpty()) {
      for (int sz = q.size(); sz > 0; --sz) {
        ...
        if (x == n - 1 && y == n - 2 && pos == Pos.HORIZONTAL.ordinal())
          return ans;
        ...
        final int newPos =
            pos == Pos.HORIZONTAL.ordinal() ? Pos.VERTICAL.ordinal() : Pos.HORIZONTAL.ordinal();
  ...
  private boolean canRotateClockwise(int[][] grid, int x, int y, int pos) {
    return pos == Pos.HORIZONTAL.ordinal() && x + 1 < grid.length && grid[x + 1][y + 1] == 0 &&
        grid[x + 1][y] == 0;
  };

  private boolean canRotateCounterclockwise(int[][] grid, int x, int y, int pos) {
    return pos == Pos.VERTICAL.ordinal() && y + 1 < grid.length && grid[x + 1][y + 1] == 0 &&
        grid[x][y + 1] == 0;
  };
}
