class Solution {
  public int checkWays(int[][] pairs) {
    ...
    for (final int u : graph.keySet())
      graph.get(u).sort((a, b) -> Integer.compare(degree[b], degree[a]));

    final int root = getRoot(degree, graph.keySet().size());
    if (root == -1)
      return 0;
    if (!dfs(graph, root, degree, connected, new ArrayList<>(), new boolean[kMax]))
      return 0;
  ...
  private int getRoot(int[] degree, int n) {
  ...
  private boolean dfs(Map<Integer, List<Integer>> graph, int u, int[] degree, boolean[][] connected,
                      List<Integer> ancestors, boolean[] seen) {
    seen[u] = true;
    ...
    for (final int v : graph.get(u)) {
      ...
      if (!dfs(graph, v, degree, connected, ancestors, seen))
        return false;
    }
