  ...
  int orangesRotting(vector<vector<int>>& grid) {
    ...
    while (true) {
      ...
      for (int i = 0; i < m; ++i)
        for (int j = 0; j < n; ++j)
          if (grid[i][j] == 1) {  // fresh
            if (isNeighborRotten(i, j, grid))  // any of 4-directionally oranges is rotten
              nextGrid[i][j] = 2;
            else
              nextGrid[i][j] = 1;
      ...
      if (nextGrid == grid) break;
      grid = nextGrid;
      ++ans;
    }


    return any_of(begin(grid), end(grid), [&](vector<int>& row) {
      return any_of(begin(row), end(row), [&](int orange) { return orange == 1; });
    }) ? -1 : ans;
  }
};