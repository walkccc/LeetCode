class Solution {
  ...
  // return A / C
  private double divide(final String A, final String C, Map<String, Map<String, Double>> graph, Set<String> seen) {
    if (A.equals(C))
      return 1.0;
    ...
    for (final String B : graph.get(A).keySet()) {
      if (seen.contains(B))
        continue;
      final double res = divide(B, C, graph, seen); // B / C
